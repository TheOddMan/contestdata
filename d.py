...
model = load_model('firsttryN1_2018_08_20',custom_objects={ 'root_mean_squared_error':root_mean_squared_error })
model.compile(loss=root_mean_squared_error, optimizer='adam')
predictions = model.predict(dscaled_zeromean,batch_size=1)

predictions_sca = ((predictions+1)/2)*(ans2dor.max(axis=(0))-ans2dor.min(axis=(0)))+ans2dor.min(axis=(0))

print(predictions_sca)    #
...
predictions = predictions_sca.ravel()
